...
const YOUTUBE_AUTH_STATE_KEY = 'voltvoice_youtube_auth_v1'
...
const hiddenUnicodeRegex = /[\u0000-\u001F\u007F-\u009F\u00AD\u034F\u061C\u115F\u1160\u17B4\u17B5\u180B-\u180E\u200B-\u200F\u202A-\u202E\u2060-\u206F\uFE00-\uFE0F\uFEFF]/g
...
const getPlainNick = (nickname, fallback = '') => {
  const normalizeNick = (value = '') => String(value || '')
    .normalize('NFKC')
    .replace(hiddenUnicodeRegex, '')
    .replace(emojiFullRegex, '')
    .replace(/[_\-.]+/g, ' ')
    // Mantener letras latinas (incluye acentos), números y espacios.
    .replace(/[^0-9A-Za-z\u00C0-\u024F ]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()

  let cleanNick = normalizeNick(nickname)
  if (!cleanNick) cleanNick = normalizeNick(fallback)
  if (!cleanNick) cleanNick = 'Usuario'

  // Evita nicks excesivamente largos que rompen render/TTS.
  return cleanNick.slice(0, 32)
}
...
export default function TikTokLivePanel({ config = {}, updateConfig, configReady = true, user = null, darkModeOverride, platformMode = 'tiktok', tokens = 0, setTokens = null, voiceGainMap = {} }) {
  ...
  const visualPlanRawCandidates = [
    String(user?.subscription?.backendPlan || '').trim().toLowerCase(),
    String(user?.subscription?.plan || '').trim().toLowerCase(),
    String(user?.plan || '').trim().toLowerCase(),
  ].filter(Boolean)
  const visualPlanRaw = visualPlanRawCandidates.find((plan) => plan !== 'free') || visualPlanRawCandidates[0] || 'free'
  const visualPlan = normalizePlanTier(visualPlanRaw)
  ...
  const displayPlan = getPlanBadgeLabel(
    String(user?.role || '').toLowerCase() === 'admin' ? 'admin' : visualPlan,
    userBillingCycle
  )
  const hasPaidAccess = currentPlan !== 'free'
  const panelTitle = isYouTubeMode ? 'YouTube Chat' : t('tiktok.panel.title')
  const planBadgeClass = currentPlan === 'admin'
    ? 'bg-red-500/20 text-red-300 border-red-400/30'
    : (currentPlan === 'pack_pro' || currentPlan === 'pack_max')
      ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400/30'
      : (currentPlan === 'pack_lite')
        ? 'bg-blue-500/20 text-blue-300 border-blue-400/30'
        : (currentPlan === 'base')
          ? 'bg-cyan-500/20 text-cyan-300 border-cyan-400/30'
          : 'bg-gray-500/20 text-gray-300 border-gray-400/30'

  const rawPlatformHelpFaq = t('tiktok.faq.questions', { returnObjects: true })
  const PLATFORM_HELP_FAQ = Array.isArray(rawPlatformHelpFaq)
    ? rawPlatformHelpFaq
    : (rawPlatformHelpFaq && typeof rawPlatformHelpFaq === 'object')
      ? Object.values(rawPlatformHelpFaq)
      : []
  ...
  const [youtubeStreamUrl, setYoutubeStreamUrl] = useState(() => String(config.youtubeStreamUrl || ''))
  const [youtubeAuthState, setYoutubeAuthState] = useState(() => {
    try {
      const raw = localStorage.getItem(YOUTUBE_AUTH_STATE_KEY)
    ...
    return { authenticated: false, channelLabel: '' }
  })
  ...
  const VOICE_LIST_KEY = 'tt_allowedUsersList'
  const VOICE_FLAG_KEY = 'tt_onlyAllowedUsers'
  ...
  const canUseAiFilter = hasPaidAccess
  const canUseAdvancedVoiceTools = hasPaidAccess
  ...
  const getYouTubeSessionLabel = (rawUrl = '') => {
    ...
    const byAt = input.match(/@([A-Za-z0-9._-]+)/)
    if (byAt?.[1]) return `@${byAt[1]}`
    const byChannel = input.match(/\/channel\/([A-Za-z0-9_-]+)/i)
    if (byChannel?.[1]) return `channel/${byChannel[1].slice(0, 10)}`
    const byHandle = input.match(/youtube\.com\/(c|user)\/([A-Za-z0-9._-]+)/i)
    if (byHandle?.[2]) return byHandle[2]
    const byVideo = input.match(/[?&]v=([A-Za-z0-9_-]{6,})/i)
    if (byVideo?.[1]) return `video:${byVideo[1].slice(0, 8)}`
  ...
  useEffect(() => {
    ...
    if (isYouTubeMode) {
      updateConfig('youtubeStreamUrl', String(youtubeStreamUrl || '').trim())
      return
    }
    const normalized = normalizeTikTokUsername(tiktokUser)
    updateConfig('lastTiktokUser', normalized || String(tiktokUser || '').trim())
  }, [tiktokUser, youtubeStreamUrl, isYouTubeMode, canWriteConfigNow, updateConfig])

  useEffect(() => {
    try {
      localStorage.setItem(YOUTUBE_AUTH_STATE_KEY, JSON.stringify(youtubeAuthState))
    } catch {
      // ignore
    }
  }, [youtubeAuthState])
    ...
    const run = async () => {
      try {
        ...
        if (data.connected) {
          setYoutubeAuthState({
            authenticated: true,
            channelLabel: String(data.channelTitle || data.channelId || 'Canal autenticado')
          })
        } else {
          setYoutubeAuthState({ authenticated: false, channelLabel: '' })
        }
      } catch {
        // ignore: keep last local state
      }
    }
    run()
  }, [isYouTubeMode])
  ...
  useEffect(() => {
    if (!isYouTubeMode || typeof window === 'undefined') return
    const params = new URLSearchParams(window.location.search)
    const authState = String(params.get('youtube_auth') || '').toLowerCase()
    if (!authState) return
    if (authState === 'success') {
      setError(null)
    } else if (authState === 'error') {
      setError('No se pudo completar la autenticacion con YouTube.')
    }
    try {
      const cleaned = new URL(window.location.href)
      cleaned.searchParams.delete('youtube_auth')
  ...
  const clearSmoothIncoming = () => {
    smoothIncomingQueueRef.current = []
    if (smoothIncomingTimerRef.current) {
  ...
  const enqueueSmoothIncoming = (items = []) => {
    ...
    smoothIncomingQueueRef.current.push(...items)
    if (smoothIncomingTimerRef.current) return

    smoothIncomingTimerRef.current = setInterval(() => {
      if (!smoothIncomingQueueRef.current.length) {
        clearInterval(smoothIncomingTimerRef.current)
        smoothIncomingTimerRef.current = null
        return
      }

      const pending = smoothIncomingQueueRef.current.length
      const take = pending > 100 ? 3 : pending > 40 ? 2 : 1
      const chunk = smoothIncomingQueueRef.current.splice(0, take)
      if (!chunk.length) return

      setMessages((prev) => [...prev, ...chunk].slice(-500))
      setStats((prev) => ({ ...prev, count: prev.count + chunk.length }))
    }, 110)
  }

  const routeIncomingMessageToVoicePipeline = (incomingMsg, { isKnownDonor = false } = {}) => {
    const msg = incomingMsg && typeof incomingMsg === 'object' ? incomingMsg : null
    if (!msg) return
    ...
    const isBanned = isUserBannedBySet(bannedRef.current, username)
    ...
    const onlyAllowedFlag = c.tt_onlyAllowedUsers !== undefined ? c.tt_onlyAllowedUsers : c.onlyAllowedUsers
    const onlyAllowedSource = c.tt_allowedUsersList !== undefined ? c.tt_allowedUsersList : c.allowedUsersList
    ...
    const onlyDonors = isEnabledFlag(c.tt_onlyDonors)
    const onlyModerators = isEnabledFlag(c.tt_onlyModerators)
    const onlySubscribers = isEnabledFlag(c.tt_onlySubscribers)
    const onlyCommunityMembers = isEnabledFlag(c.tt_onlyCommunityMembers)
    const onlyQuestions = isEnabledFlag(c.tt_onlyQuestions)
    ...
    if (c.skipRepeated) {
      const isSameUserRepeat = normalizedText === lastMessageRef.current[username]
    ...
    lastMessageRef.current[username] = normalizedText
    ...
    const priorityUser = isPriorityUser({
      ...
      isCommunityMember: !!msg.isCommunityMember
    })

    recordMessage20s()

    if (!priorityUser) {
      recordUserMessage(username)
      if (isUserSpamming(username)) {
        markFilteredMessage()
        return
      }
    }
    ...
    if (c.maxQueueEnabled && speakQueueRef.current.length >= c.maxQueueSize) { markFilteredMessage(); return }
    ...
    const overrideName = getNickOverrideValue(nickOverridesRef.current, username)
    let displayName = overrideName || nickname || username
    if (c.onlyPlainNicks || smartChatActive) {
      displayName = getPlainNick(displayName, username)
    }
    ...
    queueMessage(finalText, username, {
      id: msg.id,
      sourceText: messageForRead,
      originalMessageText: String(msg.text || ''),
      isDonor: isKnownDonor,
      isModerator: !!msg.isModerator,
      isSubscriber: !!msg.isSubscriber,
      isCommunityMember: !!msg.isCommunityMember,
  ...
  useEffect(() => {
    if (!isYouTubeMode || !isConnected) return
    const token = getAuthToken()
    if (!token) return

    let cancelled = false
    const pollAdaptiveRef = {
      emptyStreak: 0,
      errorStreak: 0,
      lastNewMessageAt: Date.now(),
    }
    const clampPollDelay = (value, max = 30000) => {
      const n = Number(value)
      if (!Number.isFinite(n) || n <= 0) return 5000
      return Math.min(max, Math.max(4000, Math.round(n)))
    }
    const getAdaptivePollDelay = ({ suggestedMs, newCount = 0, hasNew = false, hadError = false }) => {
      const suggested = clampPollDelay(suggestedMs)

      if (hadError) {
        pollAdaptiveRef.errorStreak += 1
        const backoff = Math.round(Math.max(6000, suggested) * (1.5 ** Math.min(4, pollAdaptiveRef.errorStreak)))
        return clampPollDelay(backoff, 30000)
      }

      pollAdaptiveRef.errorStreak = 0
      const now = Date.now()

      if (hasNew) {
        pollAdaptiveRef.emptyStreak = 0
        pollAdaptiveRef.lastNewMessageAt = now

        // Chat activo: modo ahorro (minimo 3s), sin estresar cuota.
        if (newCount >= 18) return clampPollDelay(Math.max(suggested, 4000))
        if (newCount >= 8) return clampPollDelay(Math.max(suggested, 4500))
        if (newCount >= 3) return clampPollDelay(Math.max(suggested, 5500))
        return clampPollDelay(Math.max(suggested, 6000))
      }

      pollAdaptiveRef.emptyStreak += 1
      const idleMs = now - pollAdaptiveRef.lastNewMessageAt
      let idleTarget = 7000
      if (idleMs >= 300000 || pollAdaptiveRef.emptyStreak >= 45) idleTarget = 30000
      else if (idleMs >= 120000 || pollAdaptiveRef.emptyStreak >= 25) idleTarget = 15000
      else if (idleMs >= 45000 || pollAdaptiveRef.emptyStreak >= 10) idleTarget = 10000

      return clampPollDelay(Math.max(suggested, idleTarget))
    }
    const scheduleNextPoll = (delayMs) => {
      if (cancelled) return
      if (youtubePollTimerRef.current) {
        clearTimeout(youtubePollTimerRef.current)
      }
      youtubePollTimerRef.current = setTimeout(pollMessages, clampPollDelay(delayMs))
    }

    const pollMessages = async () => {
      if (cancelled) return
      try {
        const response = await fetch(`${API_URL}/api/youtube/chat/messages`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        const data = await response.json().catch(() => ({}))
        ...
        const incoming = !youtubeInitialBatchConsumedRef.current
          ? incomingRaw.slice(-10)
          : incomingRaw
        youtubeInitialBatchConsumedRef.current = true
        if (!incoming.length) {
          scheduleNextPoll(getAdaptivePollDelay({ suggestedMs: data?.pollingIntervalMillis, hasNew: false }))
          return
        }

        const toAppend = []
        const knownDonorsThisPoll = new Set()
        const knownModeratorsThisPoll = new Set()
        const knownSubscribersThisPoll = new Set()
        for (const msg of incoming) {
          const msgId = String(msg?.id || '').trim()
          const text = String(msg?.text || '').trim()
          if (!msgId || !text) continue
          if (youtubeSeenIdsRef.current.has(msgId)) continue
          youtubeSeenIdsRef.current.add(msgId)
          const currentUser = String(msg?.user || 'usuario')
          const normalizedCurrentUser = normalizeTikTokUsername(currentUser)
          const isKnownModerator = Boolean(
            msg?.isModerator
            || youtubeKnownModeratorsRef.current.has(currentUser)
            || (normalizedCurrentUser && youtubeKnownModeratorsRef.current.has(normalizedCurrentUser))
            || knownModeratorsThisPoll.has(currentUser)
            || (normalizedCurrentUser && knownModeratorsThisPoll.has(normalizedCurrentUser))
          )
          const isKnownSubscriber = Boolean(
            msg?.isSubscriber
            || youtubeKnownSubscribersRef.current.has(currentUser)
            || (normalizedCurrentUser && youtubeKnownSubscribersRef.current.has(normalizedCurrentUser))
          ...
          if (msg?.isDonor) {
            const donorUser = String(msg?.user || '').trim()
            const donorUserNormalized = normalizeTikTokUsername(donorUser)
            if (donorUser) knownDonorsThisPoll.add(donorUser)
            if (donorUserNormalized) knownDonorsThisPoll.add(donorUserNormalized)
          }
          if (msg?.isModerator) {
            const moderatorUser = String(msg?.user || '').trim()
            const moderatorUserNormalized = normalizeTikTokUsername(moderatorUser)
            if (moderatorUser) knownModeratorsThisPoll.add(moderatorUser)
            if (moderatorUserNormalized) knownModeratorsThisPoll.add(moderatorUserNormalized)
          }
          if (msg?.isSubscriber) {
            const subscriberUser = String(msg?.user || '').trim()
            const subscriberUserNormalized = normalizeTikTokUsername(subscriberUser)
            if (subscriberUser) knownSubscribersThisPoll.add(subscriberUser)
            if (subscriberUserNormalized) knownSubscribersThisPoll.add(subscriberUserNormalized)
          }
        }
        ...
        sessionReceivedCountRef.current += toAppend.length
        toAppend.forEach((item) => {
          recentIncomingTimestampsRef.current = pruneRecentTimestamps([
            ...recentIncomingTimestampsRef.current,
            Date.now()
          ])
          sessionUniqueUsersRef.current.add(item.user)
          sessionUserCountsRef.current[item.user] = (sessionUserCountsRef.current[item.user] || 0) + 1
          ...
          const normalizedIncoming = normalizeMessageForMatching(item.text)
        ...
        enqueueSmoothIncoming(toAppend)
        if (knownDonorsThisPoll.size > 0) {
          setDonors((prev) => {
            const next = new Set(prev)
            knownDonorsThisPoll.forEach((entry) => next.add(entry))
            return next
          })
        }
        if (knownModeratorsThisPoll.size > 0) {
          const nextModerators = new Set(youtubeKnownModeratorsRef.current)
          knownModeratorsThisPoll.forEach((entry) => nextModerators.add(entry))
          youtubeKnownModeratorsRef.current = nextModerators
        }
        if (knownSubscribersThisPoll.size > 0) {
          const nextSubscribers = new Set(youtubeKnownSubscribersRef.current)
          knownSubscribersThisPoll.forEach((entry) => nextSubscribers.add(entry))
          youtubeKnownSubscribersRef.current = nextSubscribers
        }
        toAppend.forEach((item) => {
          const normalizedUser = normalizeTikTokUsername(item.user)
          const isKnownDonor = Boolean(
            item.isDonor
            || donorsRef.current.has(item.user)
            || (normalizedUser && donorsRef.current.has(normalizedUser))
            || knownDonorsThisPoll.has(item.user)
            || (normalizedUser && knownDonorsThisPoll.has(normalizedUser))
          )
          routeIncomingMessageToVoicePipeline({
            id: item.id,
            username: item.user,
            ...
            isTopGifter: !!item.isTopGifter
          }, { isKnownDonor })
        })
        scheduleNextPoll(getAdaptivePollDelay({
          suggestedMs: data?.pollingIntervalMillis,
          hasNew: true,
          newCount: toAppend.length,
        }))
      } catch (err) {
        if (!cancelled) setError(err?.message || 'Error leyendo chat de YouTube')
        scheduleNextPoll(getAdaptivePollDelay({ hadError: true }))
      }
    }
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        // === EVENTOS DE NOTIFICACION ===
          ...
          console.log(`[TikTok] Gift from @${giftData.username}: ${giftData.giftName}`)
        ...
        // === MENSAJES DE CHAT ===
          ...
          if (window.messagesCountSinceLastResponseRef !== undefined && !isInteractionSuppressedRef.current) {
            window.messagesCountSinceLastResponseRef++
            console.log(`[TikTok] [OK] Message counter incremented: ${window.messagesCountSinceLastResponseRef} (chat not suppressed)`)
          } else if (window.messagesCountSinceLastResponseRef !== undefined && isInteractionSuppressedRef.current) {
            console.log(`[TikTok] [SKIP] Message NOT counted (chat suppressed/bot speaking)`)
          }
          ...
          // LOGICA DE FILTRADO COMBINADO (OR entre grupos):
          ...
          // PUNTO 1: Anti-spam por usuario - si envia >5 msgs en 10s, ignorarlo temporalmente
          ...
          const unicodeCheck = normalizeUnicode(textToProcess)
          if (unicodeCheck.suspicious) {
            console.log(`[TikTok] [WARN] Mensaje sospechoso detectado: ${unicodeCheck.reason}`)
          ...
          if (c.onlyPlainNicks || smartChatActive) {
            displayName = getPlainNick(displayName, msg.username)
          }
  ...
  const getSmartMetrics = () => {
    ...
    // PUNTO 2: Ajuste dinamico con media de 20s - mas velocidad = mas filtrado
  ...
  // PUNTO 1: Anti-spam por usuario - ignora temporalmente si envia >5 msgs en 10s
  ...
  const processQueue = async () => {
    ...
    while (speakQueueRef.current.length > 0) {
      ...
      try {
            ...
            await new Promise((resolve) => {
              ...
              if (!isPreGeneratingRef.current && speakQueueRef.current.length > 0) {
                ...
                if (!authToken && !isNextLocalVoice) {
                  console.warn(`[Token Deduction] ⚠️ MISSING AUTH TOKEN (pre-gen) - voiceId: ${nextVoiceId}, username: ${nextItem.username}`)
        ...
        // ADVERTENCIA: Sin autenticación, no se deducen tokens
        if (!authToken) {
          console.warn(`[Token Deduction] ⚠️ MISSING AUTH TOKEN - voiceId: ${voiceId}, username: ${username}`)
          console.warn(`[Token Deduction] Tokens WILL NOT be deducted for this message. Please re-login to ensure proper token deduction.`)
        }
        ...
        if (
          ...
          console.log('[Pre-gen] [OK] Usando pre-generado para', item.username)
          ...
          await new Promise((resolve) => {
            ...
            audio.onended = () => {
              ...
              if (!isPreGeneratingRef.current && speakQueueRef.current.length > 0) {
                ...
                // ADVERTENCIA: Sin autenticación, no se deducen tokens
                if (!authToken) {
                  console.warn(`[Token Deduction] ⚠️ MISSING AUTH TOKEN (pre-gen #2) - voiceId: ${nextVoiceId}, username: ${nextItem.username}`)
                  console.warn(`[Token Deduction] Tokens WILL NOT be deducted for this message. Please re-login to ensure proper token deduction.`)
                }
                ...
                fetch(`${API_URL}/api/tiktok/message`, {
                  ...
                  .then(data => {
                    if (data?.audio) {
                      nextPreGeneratedRef.current = { itemId: nextItem.id, audioUrl: data.audio, voiceId: nextVoiceId, genId: preGenIdRef.current }
                      console.log('[Pre-gen] [OK] Completado para', nextItem.username)
                    }
                  })
                  .catch(() => {})
                  .finally(() => { isPreGeneratingRef.current = false })
  ...
  const handleYouTubeAuth = async () => {
    setError(null)
    const returnUrl = typeof window !== 'undefined' ? `${window.location.origin}/youtube/` : ''
    const authToken = getAuthToken()
    const tokenParam = authToken ? `&authToken=${encodeURIComponent(authToken)}` : ''
    window.location.href = `${API_URL}/api/youtube/auth/start?returnUrl=${encodeURIComponent(returnUrl)}${tokenParam}`
  }
  ...
  const handleYouTubeSignOut = async () => {
    try {
      const token = getAuthToken()
      if (token) {
        await fetch(`${API_URL}/api/youtube/chat/disconnect`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` }
        }).catch(() => {})
        await fetch(`${API_URL}/api/youtube/auth/disconnect`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` }
        })
      }
  ...
  const handleConnectSubmit = async (e) => {
    e.preventDefault()
    if (isYouTubeMode) {
      if (!youtubeAuthState.authenticated) {
        setError('Primero autentica tu cuenta de YouTube para conectar el chat.')
        return
      }
      const cleanStreamUrl = String(youtubeStreamUrl || '').trim()
      if (!cleanStreamUrl) {
        setError('Pega el link de tu stream de YouTube para conectar el chat.')
        return
      }
      setIsConnecting(true)
      setError(null)
      try {
        const token = getAuthToken()
        const response = await fetch(`${API_URL}/api/youtube/chat/connect`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            ...(token ? { Authorization: `Bearer ${token}` } : {})
          },
          body: JSON.stringify({ streamUrl: cleanStreamUrl })
        })
        const data = await response.json().catch(() => ({}))
        if (!response.ok || !data?.success) {
          setError(String(data?.error || 'No se pudo conectar al chat de YouTube'))
          return
        }
  ...
  const mobilePreviewUsername = isYouTubeMode
    ? String(connectedTikTokUser || youtubeStreamUrl || config.youtubeStreamUrl || '')
    : normalizeTikTokUsername(connectedTikTokUser || tiktokUser || config.lastTiktokUser || '')
  ...
  const visibleHelpFaq = PLATFORM_HELP_FAQ.filter((item) => {
    ...
    return `${String(item.q || '')} ${String(item.a || '')}`.toLowerCase().includes(normalizedHelpSearch)
  })
  const sessionTikTokUsername = isYouTubeMode
    ? String(connectedTikTokUser || youtubeStreamUrl || config.youtubeStreamUrl || '')
    : normalizeTikTokUsername(connectedTikTokUser || tiktokUser || config.lastTiktokUser || '')
  const sessionTikTokHandle = isYouTubeMode
    ? (sessionTikTokUsername || '')
    : (sessionTikTokUsername ? `@${sessionTikTokUsername}` : '')
  const hasYouTubeLiveLink = isYouTubeMode && !!String(youtubeStreamUrl || '').trim()
  const mobilePreviewLiveUrl = isYouTubeMode
    ? String(youtubeStreamUrl || '').trim()
    : (mobilePreviewUsername ? `https://www.tiktok.com/@${mobilePreviewUsername}/live` : '')
    ...
    <div className={`relative overflow-visible ${darkMode ? "bg-[#1a1a2e] border border-cyan-400/30 rounded-lg px-4 py-3 mb-3" : "bg-white border border-indigo-200 rounded-lg px-4 py-3 mb-3 shadow-sm"}`}>
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h2 className={`text-xs sm:text-lg font-black tracking-tight leading-tight whitespace-nowrap ${darkMode ? 'text-white' : 'text-gray-900'}`}>{panelTitle}</h2>
        ...
        <div className={`flex items-stretch rounded-xl border overflow-hidden h-9 sm:h-12 shrink-0 ${darkMode ? 'border-white/10' : 'border-slate-300'}`}>
          ...
          {sessionTikTokHandle && (
            <div className={`hidden sm:flex items-center gap-2 px-4 border-r text-sm ${darkMode
              ...
              <span className={`text-[11px] font-bold uppercase tracking-widest ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>{platformSessionLabel}</span>
              {isYouTubeMode ? (
                hasYouTubeLiveLink ? (
                  <button
                    type="button"
                    onClick={() => {
                      if (!mobilePreviewLiveUrl || typeof window === 'undefined') return
                      window.open(mobilePreviewLiveUrl, '_blank', 'noopener,noreferrer')
                    }}
              ...
              <div className={`mt-4 rounded-xl border p-4 ${darkMode ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'}`}>
                {currentPlan === 'free' ? (
                  <div className="text-center py-2">
                    <Lock className={`w-8 h-8 mx-auto mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`} />
                  ...
                  <>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <Mail className={`w-5 h-5 ${darkMode ? 'text-cyan-300' : 'text-cyan-700'}`} />
                        <div>
                    ...
                    {showSupportForm && (
                      supportDone ? (
                        <div className={`text-center py-4 rounded-xl ${darkMode ? 'bg-green-500/10 border border-green-500/30' : 'bg-green-50 border border-green-200'}`}>
                          <CheckCircle className={`w-8 h-8 mx-auto mb-2 ${darkMode ? 'text-green-300' : 'text-green-600'}`} />
      ...
      {!showConnectedView ? (
        <form onSubmit={handleConnectSubmit} className="space-y-3">
          {isYouTubeMode ? (
            <div className={`space-y-2 p-3 rounded-xl border ${darkMode ? 'bg-cyan-500/5 border-cyan-500/20' : 'bg-indigo-50 border-indigo-200'}`}>
              <div className="flex flex-col sm:flex-row gap-2">
                <button
                  type="button"
                  onClick={handleYouTubeAuth}
                  className={`sm:w-auto w-full font-semibold rounded-lg transition flex items-center justify-center gap-2 ${youtubeAuthState.authenticated
                    ? (darkMode ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 px-3 py-2' : 'bg-emerald-100 text-emerald-700 border border-emerald-300 px-3 py-2')
                ...
                {youtubeAuthState.authenticated && (
                  <button
                    ...
                    className={`sm:w-auto w-full font-semibold rounded-lg border px-3 py-2 transition ${darkMode ? 'border-white/15 text-slate-200 hover:bg-white/10' : 'border-slate-300 text-slate-700 hover:bg-slate-100'}`}
                  >
                    Cerrar autenticacion
                  </button>
                )}
              </div>
              <div className="flex gap-2 h-10">
                ...
                <button
                  type="submit"
                  disabled={isConnecting || !youtubeAuthState.authenticated || !String(youtubeStreamUrl || '').trim()}
          ...
          <p className="text-xs text-gray-400">
            {isYouTubeMode ? '1) Autentica YouTube  2) pega el link del live  3) conecta chat.' : t('tiktok.connect.bulletReadLive')}
            {!isYouTubeMode && <><br />{t('tiktok.connect.bulletSynthesize')}<br />{t('tiktok.connect.bulletPlayStreaming')}</>}
          </p>
        ...
        <div className="space-y-2">
          ...
          {/* Barra compacta de conexion - visible solo cuando no esta conectado */}
          {!isConnected && (
            <form onSubmit={handleConnectSubmit}>
              {isYouTubeMode ? (
                <div className={`space-y-2 p-3 rounded-xl border ${darkMode ? 'bg-cyan-500/5 border-cyan-500/20' : 'bg-indigo-50 border-indigo-200'}`}>
                  ...
                  <div className="flex items-center gap-2">
                    <Link2 className={`w-4 h-4 shrink-0 ${darkMode ? 'text-cyan-300' : 'text-cyan-700'}`} />
                    <input
                      type="text"
                      value={youtubeStreamUrl}
                      onChange={(e) => setYoutubeStreamUrl(e.target.value)}
                ...
                <div className={`flex gap-2 p-3 rounded-xl border ${darkMode ? 'bg-cyan-500/5 border-cyan-500/20' : 'bg-indigo-50 border-indigo-200'}`}>
                  <input
                    ...
                    value={tiktokUser}
                    onChange={(e) => setTiktokUser(e.target.value)}
                    placeholder={platformConnectPlaceholder}
                    className={`flex-1 rounded-lg px-3 py-2 text-sm focus:outline-none ${darkMode ? 'bg-[#0f0f23] border border-cyan-400/30 text-white' : 'bg-white border border-indigo-300 text-gray-900'}`}
                    disabled={isConnecting || isWaitingForLive}
                  />
                  <button
          ...
          <div className="flex-1 min-w-0 w-full">
            ...
            <div className="relative flex items-start gap-3">
              <div className="flex-1">
                    ...
                    <div className="text-center text-gray-400 py-8">
                      {isConnected ? (
                        <>
                          ...
                          <p>{emptyChatWaitingLabel}</p>
                        </>
                      ) : (
                        <>
                          <MessageCircle className="w-8 h-8 mx-auto mb-3 text-cyan-400" />
                          <p className={`font-semibold text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{emptyChatConnectTitle}</p>
                          <p className={`text-xs mt-1 ${darkMode ? 'text-gray-600' : 'text-gray-400'}`}>{emptyChatConnectHint}</p>
                        </>
                      )}
                    </div>
                  ) : (
                ...
                const badgeLabel = msg.isModerator ? 'MOD' : msg.isTopGifter ? 'TOP' : msg.isDonor ? 'DONOR' : msg.isSubscriber ? 'SUB' : msg.isCommunityMember ? 'CLUB' : null
                return (
                <div
                  key={getMessageRenderKey(msg, idx)}
          ...
          <div className={`grid grid-cols-3 sm:flex sm:flex-col rounded-lg border overflow-hidden w-full sm:w-40 sm:shrink-0 ${
            ...
            <button
              ...
              title={!canUseAiFilter ? 'Disponible con Pack Lite, Pack Pro o Pack Max' : 'Activa el filtro inteligente para eliminar spam y basura'}
            >
          ...
          {/* Panel lateral derecho - a la derecha en desktop, abajo en movil */}
            ...
            <div className={`rounded-lg border p-3 space-y-3 ${
              ...
              {[
                ...
                <div key={key} className="flex items-center gap-2">
                  ...
                  >
                    {rule.enabled && <span className="text-amber-300 text-[10px] font-bold">OK</span>}
                  </button>
