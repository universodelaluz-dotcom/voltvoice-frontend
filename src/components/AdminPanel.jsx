          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                { label: 'En L√≠nea Ahora', value: stats.onlineUsers.toLocaleString(), icon: Wifi, color: 'text-green-400', sub: '√∫ltimos 5 min', pulse: true },
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={card}>
                ...
                <p className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>AÒo: {fmtUsd(stats.revenueYearUsd)} ∑ Total: {fmtUsd(stats.revenueTotalUsd)}</p>
              </div>
              <div className={card}>
                ...
                <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Mes actual</p>
                <p className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>AÒo: {fmtUsd(stats.estimatedCostYearUsd)} ∑ Total: {fmtUsd(stats.estimatedCostTotalUsd)}</p>
              </div>
              <div className={card}>
                ...
                <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Mes actual</p>
                <p className={`text-xs mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>AÒo: {fmtUsd(stats.estimatedMarginYearUsd)}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={card}>
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>AÒo: <span className="font-bold">{stats.usersThisYear?.toLocaleString()}</span></p>
              </div>
              <div className={card}>
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>AÒo: <span className="font-bold">{fmtCompact(stats.tokensUsedYear)}</span></p>
              </div>
              <div className={card}>
                ...
                <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>AÒo: <span className="font-bold">{stats.transactionsYear?.toLocaleString()}</span></p>
                  ...
                  {stats.recentActivity.slice(0, 8).map((a, i) => (
                    <div key={i} className={`flex items-center justify-between text-xs border-b last:border-0 pb-1.5 ${darkMode ? 'border-white/5' : 'border-gray-100'}`}>
                      <div className="min-w-0">
                        ...
                        <span className={darkMode ? 'text-gray-500' : 'text-gray-400'}>{a.action || 's√≠ntesis'}</span>
                      </div>
      ...
      {generatedPassword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className={`rounded-2xl p-6 w-96 ${darkMode ? 'bg-[#1a1a35] border border-white/20' : 'bg-white border border-gray-200 shadow-xl'}`}>
            ...
            <p className={`text-xs mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Guarda esta password y comp√°rtela al usuario.</p>
