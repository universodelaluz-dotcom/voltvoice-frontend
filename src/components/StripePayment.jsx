...
export function StripePayment({ isOpen, onClose }) {
  ...
  const [loading, setLoading] = useState(null) // 'mercadopago' | 'paypal' | null
      ...
      <div className={dm
        ...
        <div className={`rounded-xl p-3 mb-5 flex items-center gap-2 ${dm ? 'bg-white/5 border border-white/10' : 'bg-indigo-50 border border-indigo-100'}`}>
          ...
          <p className={`text-sm ${dm ? 'text-gray-300' : 'text-gray-600'}`}>
            ...
            <span className="font-bold">${selectedPackage.price} USD</span> · 1 token = 1 carácter
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            {loading === 'paypal' ? '⏳ Procesando...' : (
              <>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                  <path d="M6.5 2h10C19 2 21 4 21 7c0 3.5-2.5 6-6 6.5L14.5 17h-6L7 11H5L3 2h3.5z" fill="#003087"/>
                  <path d="M7.5 3h9C18.5 3 20 5 20 7c0 3-2 5.5-5 6L14 16h-5L8 10.5H6.5L5 3H7.5z" fill="#009cde"/>
                </svg>
                Pagar con PayPal
              </>
            )}
        ...
        <p className={`text-xs mt-4 text-center ${dm ? 'text-gray-600' : 'text-gray-400'}`}>
          🔒 Pagos 100% seguros — Mercado Pago & PayPal
        </p>
      </div>
    </div>
  )
