...
`.trim()
  ...
  // === STREAMDECK PLUGIN API ===
  useEffect(() => {
    window.voltvoiceAPI = {
      pauseChat: () => {
        console.log('[StreamDeck] pauseChat called')
        chatSuppressedRef.current = true
      },
      resumeChat: () => {
        console.log('[StreamDeck] resumeChat called')
        chatSuppressedRef.current = false
      },
      setAudioMute: (muted) => {
        console.log('[StreamDeck] setAudioMute:', muted)
        if (localAudioRef.current) {
          localAudioRef.current.muted = muted
        }
        const realtimeAudio = inworldRealtimeService.getOutputAudioElement?.() || inworldRealtimeService.outputAudioElement
        if (realtimeAudio) {
          realtimeAudio.muted = muted
        }
      },
      setVoice: (voiceId) => {
        console.log('[StreamDeck] setVoice:', voiceId)
        setSelectedRealtimeVoiceId(voiceId)
        selectedRealtimeVoiceIdRef.current = voiceId
      },
      toggleCommentator: () => {
        console.log('[StreamDeck] toggleCommentator called')
        // Trigger F8 functionality
        if (isRecordingRef.current || isLoading || isPlayingResponse) {
          console.log('[StreamDeck] Bot busy, ignoring')
          return
        }
        if (!ensureAssistantReady()) {
          console.log('[StreamDeck] Assistant not ready')
          return
        }
        const intent = pickAutopilotIntent() || { type: 'react_to_chat', reason: 'streamdeck-commentator' }
        executeLocalIntent(intent)
      },
      toggleFilter: () => {
        console.log('[StreamDeck] toggleFilter called')
        updateConfig({
          ...config,
          smartChatEnabled: !config?.smartChatEnabled
        })
      },
      pushToTalk: () => {
        console.log('[StreamDeck] pushToTalk called')
      ...
      setPlaybackSpeed: (speed) => {
        ...
        console.log('[StreamDeck] setPlaybackSpeed:', validSpeed)
        updateConfig({
          ...config,
          audioSpeed: validSpeed
        })
      },
      getStatus: () => {
        return {
          connected: true,
          paused: chatSuppressedRef.current,
          muted: localAudioRef.current?.muted || false,
          voice: selectedRealtimeVoiceIdRef.current,
          speed: config?.audioSpeed || 1.0,
          filterEnabled: config?.smartChatEnabled || false,
    ...
    return () => {
      delete window.voltvoiceAPI
    }
  }, [isRecording, isLoading, isPlayingResponse, config])
