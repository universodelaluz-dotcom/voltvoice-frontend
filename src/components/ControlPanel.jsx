...
export function ControlPanel({ onClose, onGoAIRoleplay, onGoSynthesis, darkMode, config, updateConfig, user, forceUnrestricted = false }) {
  ...
  const getActualPlan = (userObj) => {
    ...
    const isAdmin = (
      ...
      Boolean(userObj?.isAdmin) ||
      Boolean(userObj?.admin) ||
  ...
  const rawPlan = getActualPlan(effectiveUser)
  const computedPlan = normalizeUserPlan(rawPlan)
  const publicPlan = normalizeUserPlan(String(effectiveUser?.plan || 'free'))
  ...
  const canUseAIAssistantInConfig = forceUnrestricted || isAdminRole || hasAddonPackActive || ['pack_lite', 'pack_pro', 'pack_max'].includes(userPlan)
  const isAIAssistantBlockedInConfig = !canUseAIAssistantInConfig
  useEffect(() => {
    if (forceUnrestricted) return
  ...
  // Lista de lectura separada por plataforma (TikTok). Fallback a la clave vieja
  // compartida para no perder la lista de usuarios que ya existian.
  const allowedUsersList = Array.isArray(config?.tt_allowedUsersList)
    ? config.tt_allowedUsersList
    : (Array.isArray(config?.allowedUsersList) ? config.allowedUsersList : [])
  const onlyAllowedUsers = config?.tt_onlyAllowedUsers !== undefined ? config.tt_onlyAllowedUsers : config?.onlyAllowedUsers
  ...
  const addAllowedUser = () => {
    ...
    updateConfig('tt_allowedUsersList', updated)
    if (!onlyAllowedUsers) updateConfig('tt_onlyAllowedUsers', true)
    setNewAllowedUser('')
  }
  ...
  const removeAllowedUser = (username) => {
    ...
    if (updated.length === 0) updateConfig('tt_onlyAllowedUsers', false)
  }
                ...
                <>
                  ...
                  <div className={`mb-2 rounded-xl px-4 py-3 border transition-colors ${
                    ...
                    <button onClick={() => {
                      ...
                      updateConfig('tt_onlyAllowedUsers', next)
