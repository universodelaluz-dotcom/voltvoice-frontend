...
export function SynthesisStudio({ onGoHome, onGoVoiceCloning, onGoControlPanel, onGoStatistics, onGoAdmin, onGoPricingPage, darkMode, setDarkMode, config, updateConfig, configReady = true, user, platformMode = 'tiktok', onSwitchPlatform, onResetPlatform }) {
              ...
              <>
                ...
                {onResetPlatform && (
                  <button
                    ...
                    title="Change default platform"
                    className={`flex items-center justify-center px-2 sm:px-3 border-r transition-all text-xs ${
                      darkMode ? 'border-white/10 bg-slate-800 text-slate-500 hover:text-slate-300 hover:bg-slate-700' : 'border-slate-300 bg-white text-slate-400 hover:text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-none stroke-current stroke-2 shrink-0" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>
                  </button>
                )}
              </>
            )}
